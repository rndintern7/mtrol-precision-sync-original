...
def load_and_sync_data(dev_upload, temp_upload):
    ...
    # Clean non-numeric characters (stripping text like '**' or 'Message')
    ...
    # Handle duplicates by averaging values for the same second
    ...
    # Synchronize: fill in temperature for every 1-second device reading
    combined['Temperature (°C)(Temp)'] = combined['Temperature (°C)(Temp)'].interpolate(method='time')
    
    # PERFORMANCE FIX: Downsample large datasets to 30,000 points to stop cursor lag
...
if device_file and temp_file:
    try:
        ...
        if available_params:
            ...
            # --- PPM CALCULATION ---
            # Using expanding window to find the absolute max/min drift across the cycle
            c_max = df[selected_param].expanding().max()
            c_min = df[selected_param].expanding().min()
            total_drift = c_max.max() - c_min.min()
            final_ppm = (total_drift * 1000000) / (TEMP_DELTA_FIXED * std["ref_range"])
            ...
            # --- GRAPH (PARAMETER + TEMP SYNCED) ---
            fig = make_subplots(specs=[[{"secondary_y": True}]])
            
            # GPU Accelerated Plot for Parameter
            ...
            # --- MATH BREAKDOWN (FIXED SYNTAX) ---
            with st.expander("🔍 View Statistics & Math"):
                st.write(f"Ref Range (Scale) Used: {std['ref_range']}")
                st.write(f"Total Drift Observed: {total_drift:.4f}")
                # Safe formatting for LaTeX
                latex_formula = r"PPM = \frac{" + f"{total_drift:.4f}" + r" \times 1,000,000}{89.85 \times " + f"{std['ref_range']}" + r"}"
                st.latex(latex_formula)
        else:
            st.error("Could not find matching Mtrol parameters in CSV.")
    except Exception as e:
        st.error(f"Error: {e}")
else:
    st.info("👈 Upload your Device CSV and Chamber_temp_data CSV to begin.")
